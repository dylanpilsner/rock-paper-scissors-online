...
.input-container{
  ...
  `;
    ...
    form.addEventListener("submit", async (e) => {
        ...
        if ((await state.getRoomInformation(target["code"].value)) ==
            "room inexistente") {
            window.alert("Esta sala no existe, por favor verifique nuevamente el código o cree una nueva sala.");
        }
        else {
            let test = await state.joinGame();
            if (test.message == "sala llena") {
                window.alert("La sala se encuentra llena, por favor ingrese a otra o cree una nueva.");
                location.reload();
            }
            else {
